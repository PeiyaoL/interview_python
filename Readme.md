...
> ​	单例模式是一种常用的软件设计模式。该模式的主要目的是确保某一个类只有一个实例存在。通过单例模式可以保证系统中一个类只有一个实例而且该实例易于外界访问，从而方便对实例个数的控制并节约系统资源。如果希望在系统中某个类的对象只能存在一个，单例模式是最好的解决方案。
...
In [48]: b = filter(lambda x: x > 2,a)

In [49]: print(b)
<filter object at 0x0000019A252C45F8>
...
[2, 4, 6]
#python3                                        
In [53]: a = map(lambda x:x+2,[1,2,3])  
                                        
In [54]: a                              
Out[54]: <map at 0x19a252cef60>         
...
```python
>>> reduce(lambda x,y:x*y,range(1,4))  #Python2
6
In [43]: from functools import reduce  #Python3中reduce()函数被放置在functools模块里

In [44]: reduce(lambda x,y:x+y,[1,2,3]) #计算列表和：1+2+3
Out[44]: 6

In [45]: reduce(lambda x,y:x*y,[1,2,3])  #计算列表乘积，即3！=1*2*3
Out[45]: 6
```
...
In [60]: b = a #直接赋值,传对象的引用

In [61]: c = copy.copy(a) #浅拷贝:拷贝父对象,不会拷贝子对象,c和a的子对象指向同一个引用

In [62]: d = copy.deepcopy(a) #深拷贝:拷贝父对象和子对象

In [63]: a.append(5)  #修改a的父对象

In [64]: a[4].append("PeiyaoL")  #修改a的子对象

In [65]: print(" a = {}\n b = {}\n c = {}\n d = {}".format(a,b,c,d))
 a = [1, 2, 3, 4, ['a', 'b', 'PeiyaoL'], 5]
 b = [1, 2, 3, 4, ['a', 'b', 'PeiyaoL'], 5]
 c = [1, 2, 3, 4, ['a', 'b', 'PeiyaoL']]
 d = [1, 2, 3, 4, ['a', 'b']]
